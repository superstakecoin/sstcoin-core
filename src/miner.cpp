...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SuperStakeCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SuperStakeCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SuperStakeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sstcoin-miner");
   ...
   // Each thread has its own key and counter
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SuperStakeCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
