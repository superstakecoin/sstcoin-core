...
#if defined(NDEBUG)
#error "SuperStakeCoin cannot be compiled without assertions."
#endif
...
CScript GetAdvisorsFeeScript1() {
    CBitcoinAddress devAddress = CBitcoinAddress("bJ4FAboxj59vnRC2E68XyWezzeCdfjozbf");
    return GetScriptForDestination(devAddress.Get());
}


CScript GetAdvisorsFeeScript2() {
    CBitcoinAddress devAddress = CBitcoinAddress("bQEpD2HE8pyijj81V2wN7HK7uhjVn66k8L");
    return GetScriptForDestination(devAddress.Get());
}
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK()) {
        ...
        // POS subsidy
        int halvings = nHeight / (262800 * 4);

        if (halvings >= 43)
            return 0;

        int64_t nSubsidy = 43 * COIN;
...
CAmount GetAdvisorsFee1(int64_t blockValue) {
	return blockValue * 0.1;
}

CAmount GetAdvisorsFee2(int64_t blockValue) {
	return blockValue * 0.1;
}
    ...
    if (block.IsProofOfStake()) {
        ...
        for (const auto& tx : block.vtx) {
            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                 if (output.scriptPubKey == GetAdvisorsFeeScript1() && output.nValue == GetAdvisorsFee1(GetBlockValue(pindex->pprev->nHeight))) {
                     bFound1 = true;
                 }
                 if (output.scriptPubKey == GetAdvisorsFeeScript2() && output.nValue == GetAdvisorsFee2(GetBlockValue(pindex->pprev->nHeight))) {
                     bFound2 = true;
                 }
            }
        }
